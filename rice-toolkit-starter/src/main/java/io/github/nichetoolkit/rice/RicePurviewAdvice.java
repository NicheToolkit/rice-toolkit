...
/**
 * <p>RestUserAdvice 自定义登录用户权限信息参数解析器</p>
...
public interface RicePurviewAdvice {
    ...
    default boolean supports(MethodParameter parameter) {
        RestPurview restPurview = parameter.getParameterAnnotation(RestPurview.class);
        return GeneralUtils.isNotEmpty(restPurview);
    }

    /**
     * 解析当前登录用户信息
     * 只有{@link RicePurviewAdvice#supports(MethodParameter)}方法返回true才会执行此方法
     * @param parameter 标注了{@link RestPurview}注解的参数对象，通过参数对象可以获得到注解实例
